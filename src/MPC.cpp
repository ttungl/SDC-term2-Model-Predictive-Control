...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int delta_cst_gap = 100;
    const int a_cst_gap = 10;
    
    fg[0] = 0; 

    // the cost based on the ref state
    for (int i = 0; i < N; i++) {
      fg[0] += cte_cst * CppAD::pow(vars[cte_start + i], 2);
      fg[0] += epsi_cst * CppAD::pow(vars[epsi_start + i], 2);
      fg[0] += v_cst * CppAD::pow(vars[v_start + i] - ref_v, 2);
    }

    // minimize the actuators use.
    for (int i = 0; i < N - 1; i++) {
      fg[0] += delta_cst_use * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += a_cst_use * CppAD::pow(vars[a_start + i], 2);
    }

    // minimize the value gap between actuators
    for (int i = 0; i < N - 2; i++) {
      fg[0] += delta_cst_gap * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += a_cst_gap * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  // reference: https://discussions.udacity.com/t/car-does-not-move/267302/8
  ...
  // Steering angle variable upper and lower limits
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }

  // Throttle acceleration variable upper and lower limits
