...
// TODO: Set the timestep length N and elapsed duration between timesteps dt
size_t N = 100;
double dt = 0.01; 
...
// The state and actuator variables in a singular vector.
size_t x_start = 0;
...
size_t act_start = delta_start + N -1;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //**
    // The cost is added to fg[0], and additional cost is added to fg[0]
    fg[0] = 0;
    ...
    for (int i=0; i<N-1; i++){
      ...
      fg[0] += 5*CppAD::pow(vars[act_start +i], 2);
      fg[0] += 700*CppAD::pow(vars[delta_start + i]*vars[v_start + i], 2); // penalty for steering and speed.
    }

    for (int i=0; i< N-2; i++){
      fg[0] += 200*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 10*CppAD::pow(vars[act_start + i + 1] - vars[act_start + i], 2);
    }

    // constraints.
    // Add one to each starting indices.
    ...
    for (int j=1; j < N; j++){
      ...
      AD<double> act = vars[act_start + j - 1];
      AD<double> delta = vars[delta_start + j - 1];
      
      //
      if (j>1){
        act = vars[act_start + j - 2];
      ...
      // Constraint x to be zero.
      // Compute derivatives and pass to the solver.
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // non-actuators: set lower and upper limits for variables.
  for (int i=0; i < delta_start; i++){
    vars_lowerbound[i] = -1.0E19;
    vars_upperbound[i] = 1.0E19;
  }

  // delta: set lower and upper limits for variables.
  for (int i=delta_start; i < act_start; i++){
    vars_lowerbound[i] = -0.43;
    vars_upperbound[i] = 0.43;
  }

  // acceleration: set lower and upper limits for variables.
  for (int i=delta_start; i < act_start; i++){
  ...
  result.push_back(solution.x[act_start]);
